...
public class WebSocketHander extends TextWebSocketHandler {
	...
	// 用户标识
	private static final String USER_ID = "WEBSOCKET_USERID"; // 对应监听器的key
	...
	/**
	 * 连接成功时候，会触发页面上onopen方法【初次链接成功执行】
	 */
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {

		System.out.println("成功建立websocket连接!");
		String userName = (String) session.getAttributes().get(USER_ID);//USER_ID=WEBSOCKET_USERID
		// 使用userName区分WebSocketHandler，以便定向发送消息
		users.put(userName, session);//userName：Httpsession存储的登录名
